...
# https://wonder.cdc.gov/controller/saved/D76/D69F994
library(readr)
deaths = read_tsv("./PSet4-data/Overdose-1999-2017.txt")
#deaths = read_tsv("./PSet4-data/AIDS-1999-2017.txt")
head(deaths)
```
```{r}
df = deaths %>%
    ...
    ungroup() %>%
    mutate(state = tolower(State), value = Deaths)
head(df)
```
...
ggplot(df, aes(map_id = state)) + 
  ...
  labs(
    title = "US Per Capita Income by States, 1974", 
    x = "longitude", 
    y = "latitude",
    caption = "source: https://www.rdocumentation.org/packages/datasets/versions/3.6.1/topics/state"
  ) +
  coord_fixed(1.3) +
