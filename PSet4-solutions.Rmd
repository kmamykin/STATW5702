...
df[df>80] <- NA
vis_miss(df) + ggtitle("Missing values in the dataset") #Plot 1

gg_miss_var(df)+ ggtitle("Missing values in the dataset sorted by count") #Plot 2

```
Plot 1 shows the missing values (by column) in the dataset in the order they appear, X1 - X10. As the legend shows, the black parts of the visualization represent the missing values and the white represents the present values. The y axis shows the order of observations as they appear in the dataset. 
...
```{r}
df1 <- data.frame(t(df))
vis_miss(df1) + ggtitle("Missing values in the dataset")
gg_miss_var(df1)+ ggtitle("Missing values in the dataset rows sorted by count")
#have to change rownames
```
Rows Y7, Y4 and Y10 have the highest number of missing values.
