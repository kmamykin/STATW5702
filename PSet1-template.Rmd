...
The bar chart highlights that during the months of August and September the forests are most affected by fires, and about 81% of the total burned area is burned furing those months. 

### Appendix

Below are some other visualizations of the area by month.

This boxplot gives a better understanding of the distribution of fire areas, but the downside is that we have to hse a log10 scale since the median of all fires is 0.52 ha and the outliers that boxplot is designed to highlight are > 100 ha.

```{r}
ggplot(data = ff) +
  geom_boxplot(aes(x = month, y = area)) + 
