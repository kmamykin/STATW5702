...
ggplot(data = abalone, aes(x=whole_weight, y =..density..)) + 
  geom_histogram(bins = 35, color= "black", fill='blue') + 
...
ggplot(abalone, aes(x = reorder(sex, -whole_weight, median),y = whole_weight))+
        geom_boxplot(fill='blue', color='black', width = 0.2)+
...
g1 <- ggplot(hepatitis, aes(x=age, y=..density..)) +
  geom_histogram(binwidth =  5, color='black', fill = 'blue', closed = "left", boundary = 0 )+
  labs(x='Age (Left inclusive)',y='')

g2 <- ggplot(hepatitis, aes(x=age, y=..density..)) +
  geom_histogram(binwidth  = 5, color='black', fill = 'blue', closed = "right", boundary = 0 )+
...
glass_values <- gather(glass, key = "variable", value = "value", glass_variables) %>%
